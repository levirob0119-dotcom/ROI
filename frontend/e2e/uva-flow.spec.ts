...
test('UVA Analysis Full Flow', async ({ page, request }) => {
    ...
    await expect(page.getByPlaceholder('例如：2026Q1 满意度优化')).toBeVisible();

    const projectTimestamp = new Date().getTime();
    const projectName = `E2E Test ${projectTimestamp} `;
    await page.getByPlaceholder('例如：2026Q1 满意度优化').fill(projectName);
    ...
    await expect(page.getByPlaceholder('例如：2026Q1 满意度优化')).not.toBeVisible({ timeout: 5000 });
